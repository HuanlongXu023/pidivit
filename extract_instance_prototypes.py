...
def main(model='vitl14', dataset='fs_coco17_support_novel_30shot', use_bbox='yes',
         epochs=1, device=0, n_clst=5, split=0, idx=0, out_dir=None, use_aug='no'):
    use_bbox = use_bbox == 'yes'
    use_aug = use_aug == 'yes'  # 添加这一行
        ...
        for _ in range(epochs):
            for item_i, item in enumerate(dataloader):
                ...
                if 'stuff' in dataset_name:
                    ...
                    # 检查是否有 gt_masks，如果没有则使用 bbox
                    if hasattr(instances, 'gt_masks') and instances.gt_masks is not None:
                        masks_used = instances.gt_masks.tensor
                    else:
                        # 如果没有mask，直接使用bbox创建mask
                        masks_used = to_mask(instances.gt_boxes.tensor,
                                             image.shape[1], image.shape[2]).to(device)
