...
def main(inp, 
    ...
    print(f'Original unique labels: {unique_labels.tolist()}')
    ...
    for old_id, new_id in label_mapping.items():
        labels_remapped[labels == old_id] = new_id
    labels = labels_remapped

    # 如果有thing_classes，也需要重新排序
    if not is_background_tokens and len(thing_classes) > 0:
        # 根据映射重新排序thing_classes
        old_thing_classes = thing_classes
        thing_classes = [old_thing_classes[old_id] for old_id in sorted(label_mapping.keys())]
        print(f'Reordered thing_classes: {thing_classes}')
